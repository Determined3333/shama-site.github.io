...
document.addEventListener("click", e => {
  ...
  const langMenus = document.querySelectorAll(".header__lang-menu");
  const allButtons = document.querySelectorAll(".header__lang-btn");

  // Если клик вне меню и кнопки — закрыть все
  if (!btn && !e.target.closest(".header__lang-menu")) {
    langMenus.forEach(menu => menu.classList.remove("active"));
    allButtons.forEach(b => b.classList.remove("active"));
    return;
  }
  if (btn) {
    const parent = btn.closest(".header__lang");
    const menu = parent.querySelector(".header__lang-menu");
    const isActive = btn.classList.toggle("active");

    // Закрываем остальные
    langMenus.forEach(m => m !== menu && m.classList.remove("active"));
  ...
  if (langItem) {
    const parent = langItem.closest(".header__lang");
    const menu = parent.querySelector(".header__lang-menu");
    const btn = parent.querySelector(".header__lang-btn");

    // Закрыть меню
    menu.classList.remove("active");
    btn.classList.add("active"); // SVG остаётся окрашенным
  }
});
...
__webpack_require__.r(__webpack_exports__);
// const playBtn = document.querySelector('.about__video-btn');
// const modal = document.getElementById('videoModal');
// const video = modal.querySelector('.video-modal__video');
// const loader = modal.querySelector('.video-modal__loader');
...
//   // Когда видео готово — скрываем лоадер
...
// modal.querySelector('.video-modal__overlay').addEventListener('click', closeVideo);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  playBtn.addEventListener('click', () => {
    ...
    video.addEventListener('loadedmetadata', () => {
      ...
      video.classList.toggle('vertical', aspect < 1);
    });

    // Основной фикс — слушаем 'loadeddata' и 'playing'
    const showVideo = () => {
      loader.style.display = 'none';
      video.style.display = 'block';
    };
    ...
    video.play().catch(err => {
      console.warn('Видео не смогло запуститься автоматически:', err);
      showVideo();
    });
  });
  function closeVideo() {
...
__webpack_require__.r(__webpack_exports__);
// document.addEventListener("DOMContentLoaded", () => {
//   const audio = document.querySelector(".why__voice-audio");
//   const btn = document.querySelector(".why__voice-btn");
//   const bars = document.querySelectorAll(".why__voice-bars rect");
...
//         bar.setAttribute("fill", "#a1be6b"); // зелёный
//       } else {
//         bar.setAttribute("fill", "#D9D9D9"); // серый
//       }
//     });
...
//     bars.forEach(bar => bar.setAttribute("fill", "#D9D9D9")); // сбросить
//     btn.classList.remove("playing");
//     isPlaying = false;
//   });
// });
...
//   const audio = document.querySelector(".why__voice-audio");
//   const btn = document.querySelector(".why__voice-btn");
//   const svg = document.querySelector(".why__voice-bars");
//   const bars = document.querySelectorAll(".why__voice-bars rect");

//   let isPlaying = false;

//   // Рассчёт прогресса и закрашивание
//   function updateBars() {
//     const progress = audio.currentTime / audio.duration;

//     bars.forEach(bar => {
//       // Получаем координаты палочки
//       const x = parseFloat(bar.getAttribute("x"));
//       const width = parseFloat(bar.getAttribute("width"));
//       const svgWidth = parseFloat(svg.getAttribute("width"));

//       // Если центр палочки <= текущего прогресса, красим зелёным
//       const center = x + width / 2;
//       if (center / svgWidth <= progress) {
//         bar.setAttribute("fill", "#a1be6b");
//       } else {
//         bar.setAttribute("fill", "#D9D9D9");
//       }
//     });
...
//   // Реализация seek по клику на SVG
//   svg.addEventListener("click", e => {
//     const rect = svg.getBoundingClientRect();
//     const clickX = e.clientX - rect.left;
//     const svgWidth = parseFloat(svg.getAttribute("width"));
//     const seekTime = (clickX / svgWidth) * audio.duration;
//     audio.currentTime = seekTime;

//     // Обновляем закрашивание сразу
//     updateBars();
//   });

//   audio.addEventListener("ended", () => {
//     bars.forEach(bar => bar.setAttribute("fill", "#D9D9D9"));
//     btn.classList.remove("playing");
//     isPlaying = false;
//   });
// });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Перемотка по клику на SVG
  svg.addEventListener("click", e => {
    const rect = svg.getBoundingClientRect();
    const clickX = e.clientX - rect.left;
    const svgWidth = parseFloat(svg.getAttribute("width"));
    audio.currentTime = clickX / svgWidth * audio.duration;
    updateBars();
  });
  audio.addEventListener("ended", () => {
    bars.forEach(bar => bar.setAttribute("fill", "#D9D9D9"));
...
/* harmony import */ var _voice_block_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./_voice-block.js */ "./src/js/_voice-block.js");
/* harmony import */ var _video_block_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./_video-block.js */ "./src/js/_video-block.js");
/* harmony import */ var _reviews_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./_reviews.js */ "./src/js/_reviews.js");
/* harmony import */ var _lang_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./_lang.js */ "./src/js/_lang.js");
